...
class BrowserApp(ctk.CTk):
    ...
    def _build(self):
        ...
        # Inject JS URL interceptor once WebView2 core is ready, then poll
        self._js_injected=False
        self.after(2000,self._inject_url_hook)
        self.after(2500,self._poll_url)
    ...
    # ── Inject JS hook to intercept YouTube SPA navigation ──
    def _inject_url_hook(self):
        try:
            js="""(function(){
if(window._ytdrop_hooked) return;
window._ytdrop_hooked=true;
window._ytdrop_url=location.href;
var op=history.pushState, or=history.replaceState;
history.pushState=function(){op.apply(this,arguments);window._ytdrop_url=location.href;};
history.replaceState=function(){or.apply(this,arguments);window._ytdrop_url=location.href;};
window.addEventListener('popstate',function(){window._ytdrop_url=location.href;});
setInterval(function(){window._ytdrop_url=location.href;},300);
})();"""
            self._browser.evaluate_js(js)
            self._js_injected=True
        except: pass

    # ── URL Polling (reads from JS-side cached URL) ──
    def _poll_url(self):
        try:
            def _update_ui(url):
            ...
            # Use the JS-cached URL variable for faster detection
            script = "window._ytdrop_url || window.location.href"
            self._browser.evaluate_js(script, callback=_cb)
        except: pass
        self.after(500,self._poll_url)
